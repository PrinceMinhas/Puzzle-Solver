...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return True if and only if there is one empty position that can not be
        filled because an instance of each symbol has been used in the same
        ...
        for i in range(len(self._symbols)):
            if self._symbols[i] == "*":
                row_set = self._row_set(i)
                column_set = self._column_set(i)
                subsquare_set = self._subsquare_set(i)
                if all([(x in row_set or x in column_set or x in subsquare_set)
